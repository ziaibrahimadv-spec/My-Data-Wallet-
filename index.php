...
#pw-app .grp>.sh h3{font-size:1.5rem;font-weight:600;letter-spacing:-.01em;color:#FFFFFF;line-height:1.3;text-align:left}
#pw-app .grp>.sh .sh-sub{font-size:13.5px;font-weight:400;color:var(--sub);line-height:1.4;letter-spacing:.1px;text-align:left}
...
#pw-app #ntBody{font-size:16.5px;line-height:1.75;color:var(--tx2);white-space:pre-wrap;word-wrap:break-word;max-height:55vh;overflow-y:auto;min-width:280px}
...
#pw-app .modal-notes-edit{margin-top:22px}
#pw-app .modal-notes-edit .label{font-size:12px;font-weight:700;letter-spacing:2px;color:var(--mu);margin-bottom:10px;text-transform:uppercase}
#pw-app .md.editing .modal-notes{display:none!important}
...
<div class="modal-notes-edit" id="mnotesEditWrap" style="display:none">
  <div class="label">Notes</div>
  <textarea class="text-edit" id="mnotesEdit" placeholder="Add notes about this card…"></textarea>
</div>
</div></div>
<div class="mf">
...
const mnotesEditWrap=$('#mnotesEditWrap');
const mnotesEdit=$('#mnotesEdit');
...
mnotesEdit.value = String(currentCard.notes||'');
exitEditMode(true);
...
mtext.textContent=currentCard.prompt;
current=currentCard.prompt;
if(mnotesTxt) mnotesTxt.textContent = String(currentCard.notes||'');
const nb=$('#mnotes'); if(nb) nb.style.display = currentCard.notes && currentCard.notes.trim() ? '' : 'none';
}
};
const checkChanges=()=>{
...
mnotesEdit.value.trim()!==String(currentCard.notes||'').trim();
...
const newNotes = mnotesEdit.value.trim();
...
[mtitleEdit,mdescEdit,mtextEdit,mnotesEdit].forEach(el=>el.addEventListener('input',checkChanges));
